...
class Auth():
    ...
    def login(self):
        while True:
            ...
            if user and user.password == password:
                self.auth_view.display_message("\nLogin successful!", sleep_time=2)
                return user
            else:
                self.auth_view.display_message("\nIncorrect username or password.", sleep_time=2)
    ...
    def register(self):
        ...
        if self.db.write(new_user_dict):
            self.auth_view.display_message(f"\nUser '{username}' registered successfully!", sleep_time=2)
        else:
            self.auth_view.display_message("\nAn error occurred while creating the user. Please try again.", sleep_time=2)
            del new_user
